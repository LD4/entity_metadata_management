...
_Create_{:.term} _SHOULD_{:.strong-term} be used when the entity is new in the source dataset and available for use. A provider MUST NOT use _Create_{:.term} to broadcast that an entity exists if they don't want consumer to use the entity yet; see _Add_{:.term} below.
